...
\\ \\
Grâce à cela, il est déjà possible de répondre à certaines questions, comme par exemple trouver le chemin le plus court entre deux noeuds. De répondre à certains problèmes comme le parcours de chaque noeud ( et respectivement de chaque lien), tout en limitant le nombre de liens (et respectivement de noeuds) parcourus. L'ensemble des algorithmes de la théorie des graphes nous sert donc comme base pour nos algorithmes d'études de réseaux.
\\ \\ 
Dans un second temps, la théorie des réseaux complexes définit plusieurs mesures comme le nombre de sauts qui correspondent au nombre de noeud parcouru, ou encore la distribution des degrés, c'est à dire la probabilité qu'un noeud, choisi de manière aléatoire, comporte un certain degré. Le degré de séparation est également une notion importante qui définit le nombre de noeuds minimum pour aller d'un noeud à un autre.
\begin{figure}[h]
\centering
...
$ y = ax^k $
Dans un tel réseau, la distribution des degrés a une in-variance d'échelle, c'est à dire qu'aucune échelle ne caractérise la distribution des degrés.
\\ \\ 
